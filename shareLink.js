define( ["qlik", "jquery"],
function (qlik, $) {
	return {
		support : {
			snapshot: false,
			export: false,
			exportData : false
		},
		definition: {
			type: "items",
			component: "accordion",
			items: {
				settings: {
					uses: "settings"
				},
				about: {
					label: "About"
					,type: "items"
					,items: [
						{
							component: "radiobuttons",
							type: "string",
							defaultValue: "icon",
							options: [
							  {
								"label": "Share Icon",
								"value": "icon"
							  },{
								"label": "As Button",
								"value": "button"
							  },{
								"label": "Text",
								"value": "text"
							  }
							],
							ref: "rb1",
							label: "Link Style"
						},{
							label: "Link Text",
							component: "string",
							expression: "optional",
							ref: "linktext",
							defaultValue: "Share This"
						},{
							label: "by Christof Schwarz", 
							component: "text"
						},{
							label: "Open on Github",
							component: "button", 
							action: function(arg) {
								window.open('https://github.com/ChristofSchwarz/qs-ext-miniShareLink','_blank');
							}
						}
					]
				}
			}
		},
		paint: function ($element, layout) {
			var self = this; 
			var ownId = this.options.id;             
			var app = qlik.currApp(this);
			var sheet = qlik.navigation.getCurrentSheetId();
			console.log(layout);
			$element.html( '<a id="click' + ownId + '"' 
			+ (layout.rb1=='button'?' class="lui-button"':'')
			+ (layout.rb1=='text'?' style="text-decoration:underline;"':'')  
			+ '>'
			+ (layout.rb1=='icon'?'<img width="24" height="24" src="data:image/png;base64,"/>'
				:layout.linktext)
			+ '</a>');
			$('#click' + ownId).click(function(){
				var qFormula = "GetCurrentSelections(']/select/', '/[', '];[', 1000)";
				app.model.enigmaModel.evaluate(qFormula).then(function(result) {
					var urlArr = document.location.href.split('/');
					var url = urlArr.splice(0, urlArr.indexOf('sheet') + 2).join('/') + '/select/' + encodeURI(result) + '%5D';
					url = url.replace('/select/-%5D','');
					if ( window.prompt("Copy this link - click OK to open in mail", url)) {
						window.open('mailto:?body='+escape(url),'_new');
					};
				})	
			});
			return qlik.Promise.resolve();
		}
	};
});
